...
public interface LedgersRepo extends JpaRepository<Ledger, Integer> {
    @Query(value = "select new com.xdf.xd_f371.dto.LedgerDto(ld.ledger_id,ld.id, l.bill_id,l.amount,l.from_date,l.end_date,l.status,l.so_km,l.giohd_md,l.giohd_tk," +
            ...
            "ld.phai_nhap,ld.haohut_sl,l.loainv,ld.nl_gio,ld.nl_km,ld.soluong_px) from Ledger l join l.ledgerDetails ld where l.bill_id=:bId and l.loai_phieu like :lp and l.from_date between :sd and :ed ")
    List<LedgerDto> findLedgerByBillIdAndQuarter_id(@Param("bId") Long billId,@Param("sd") LocalDate sd,@Param("ed") LocalDate ed,@Param("lp") String lp);
